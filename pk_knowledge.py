...
def kb_context(query, max_chars=4500):
    ...
    if not dhamma:
        ...
        return (
            ...
            "URL වල මාතෘකා වචනවල අකුරු වැරදි තිබේ නම් නිවැරදි කරන්න (උදා: widows→windows) — URL ආරම්භය නොවෙනස්ව තබා මාතෘකා වචන විතරක් මාරු කරන්න, වෙන URL හැඩයක් නොහදන්න. "
            "සුහද හරියවස් කතාවක් නම් (උදා: ආයුබෝවන්න, ඔයා කවුද) links දෙන්න එපා:\n"
            f"▶️ YouTube: {yt}\n"
            f"📖 විකිපීඩියා: {wp}"
        )
    
    # v1.9: පොත් injection තාවකාලිකව off (owner decision 2026-09-16) — SILA නොවෙනස්
    return ""
    parts = [
